...
export type ArpcImprovementChartProps = {
	readonly alternatives?: ReadonlyArray< AlternativeTrackingData >;
	readonly goal: GoalId;
	readonly unique: boolean;
};

export const ArpcImprovementChart = (
	props: ArpcImprovementChartProps
): JSX.Element => {
...
const getOptions = (): ChartOptions => {
	return {
		...
		plugins: {
			title: {
				display: true,
				text: _x( 'ARPC Improvement', 'text', 'nelio-ab-testing' ),
				color: '#464646',
				font: {
					size: 14,
					style: 'normal',
...
const getData = (
	...
	const originalValue = unique
		? ( alternatives[ 0 ]?.uniqueValues[ goal ] || 0 ) /
				( alternatives[ 0 ]?.uniqueConversions[ goal ] || 0 ) || 0
		: ( alternatives[ 0 ]?.values[ goal ] || 0 ) /
				( alternatives[ 0 ]?.conversions[ goal ] || 0 ) || 0;
