...
export type ConversionImprovementChartProps = {
	readonly alternatives?: ReadonlyArray< AlternativeTrackingData >;
	readonly goal: GoalId;
	readonly unique: boolean;
};

export const ConversionImprovementChart = (
	props: ConversionImprovementChartProps
): JSX.Element => {
	...
	const colors = data.map( ( value ): string =>
		0 > value ? RED : NORMAL
	);
...
const getData = (
	...
	alternatives.map( ( alternative ) =>
		unique
			? alternative.uniqueImprovementFactors[ goal ] || 0
			: alternative.improvementFactors[ goal ] || 0
	);
